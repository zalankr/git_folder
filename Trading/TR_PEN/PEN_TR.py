...
order = order_time() 
if order['round'] == 0:
    TA.send_tele(f"Pension: 매매시간이 아닙니다.")
    sys.exit(0)
message.append(f"Pension: {order['date']}, {order['time']}, {order['round']}/{order['total_round']}회차 매매 시작")
...
if order['round'] == 1:
    ...
    if not isinstance(account, dict):
        TA.send_tele(f"Pension: 총 원화평가금 조회 불가로 종료합니다. ({account})")
        sys.exit(1)
    total_krw_asset = account['total_krw_asset']  # nass_amt (주식평가금 + D+2 현금 합계)
    message.append(f"Pension총자산: {int(total_krw_asset):,}원 (주식:{int(account['stock_eval_amt']):,} + 현금:{int(account['cash_balance']):,})")

    # 당일 target 투자금액 및 목표 산출 및 저장하기
    this_krw_asset = total_krw_asset
    message.append(f"Pension 이번 투자기준금: {int(this_krw_asset):,}원")
...
if len(sell_code) == 0:
    message.append("Pension:매도 종목 없음")

elif sell_split[0] > 0:
    message.append(f"Pension: {order['round']}회차 - 매도 주문")
    for code, qty in sell.items():
        ...
        for i in range(local_split_count):
            ...
            if order_info is None:
                message.append(f"Pension 매도 오류: {code} API 응답 없음")
    ...
    message.append(f"Pension: {order['round']}회차 - 매도 주문 없음 (분할횟수 0)")
...
if not isinstance(KRW, (int, float)):
    TA.send_tele(f"Pension: 주문가능현금 조회 불가로 종료합니다. ({KRW})")
    sys.exit(1)
...
message.append(
    f"Pension매수가능: {int(orderable_KRW):,}원 | 목표매수금: {int(target_KRW):,}원"
...
if target_KRW > orderable_KRW:
    ...
    message.append(f"Pension 매수수량 조정 완료 (adjust_rate={adjust_rate:.4f})")
else:
    message.append("Pension 매수가능금 충분 → 수량 조정 없음")
...
if len(buy_code) == 0:
    message.append("Pension:매수 종목 없음")

elif len(buy_code) > 0 and buy_split[0] > 0:
    message.append(f"Pension: {order['round']}회차 - 매수 주문")
    for code, qty in buy.items():
        ...
        if split_qty < 1:
            if qty < 1:
                message.append(f"Pension매수 스킵: {code} 수량 0주 (조정후 제거대상)")
                continue
            local_split_count = 1
        ...
        if not isinstance(price, int) or price == 0:
            TA.send_tele(f"Pension: 현재가 없음으로 종료합니다. ({code})")
            sys.exit(1)        
        ...
        for i in range(local_split_count):
            ...
            if order_info is None:
                ...
                order_info = KIS.order_buy_KR(code, this_qty, order_price, "00")
            if order_info is None:
                message.append(f"Pension매수 오류: {code} {this_qty}주 {order_price:,}원 API 응답 없음")
            elif order_info.get("success"):
...
if order['round'] == 12:
    ...
    cancel_message = cancel_orders(side='all')
    message.append(cancel_message)
    message.append(f"Pension {order['date']} 리밸런싱 종료")
    ...
    if not isinstance(stocks, list):
        ...
        sys.exit(1)
    if len(stocks) < 7:
        TA.send_tele(f"Pension: 잔고 종목 수가 target 종목수 7개보다 작음. ({len(stocks)})")
    ...
    # telegram message
    message.append(
        f"📊 Pension 최종잔고 {tele_data['total']['date']}\n"
