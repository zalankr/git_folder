...
try:
    ...
    TA.send_tele("Pension: 이미 실행 중입니다.")
    sys.exit(0)

# KIS instance 생성
key_file_path = "/var/autobot/KIS/kis44334640nkr.txt"
token_file_path = "/var/autobot/KIS/kis44334640_token.json"
cano = "44334640"   # 계좌번호 (숫자만 입력, 하이픈(-) 제외)
acnt_prdt_cd = "22" # 연금저축계좌
KIS = KIS_PEN.KIS_API(key_file_path, token_file_path, cano, acnt_prdt_cd)

PEN_result_path = "/var/autobot/TR_PEN/PEN2_result.json" # json
PEN_target_path = "/var/autobot/TR_PEN/PEN2_target.json" # json
...
def health_check():
    ...
    if not KIS.access_token:
        checks.append("Pension체크: API 토큰 없음")
    ...
    try:
        ...
        checks.append("Pension체크: KIS API 서버 접속 불가")
        
    # 3. 거래가능일 체크
    checkday = KIS.is_KR_trading_day()
    if checkday == False:
        checks.append("Pension체크: 거래일이 아닙니다.")
...
def target_invest(target: dict, this_krw_asset: float):
    ...
    if abs(total_weight - 1.0) > 0.01:   # 1% 오차 허용
        TA.send_tele(f"Pension경고: weight 합계 = {total_weight:.3f} (1.0 아님). 계속 진행합니다.")
    ...
    for i in target_code:
        ...
        if price == 0 or not isinstance(price, int):
            TA.send_tele(f"Pension: 현재가 조회 불가로 종료합니다. ({price})")
            sys.exit(1)
...
def hold_invest():
    ...
    if not isinstance(stocks, list):
        TA.send_tele(f"Pension: 잔고 조회 불가로 종료합니다. ({stocks})")
        sys.exit(1)
...
order = order_time() 
if order['round'] == 0:
    TA.send_tele(f"Pension2: 매매시간이 아닙니다.")
    sys.exit(0)
message.append(f"Pension2: {order['date']}, {order['time']}, {order['round']}/{order['total_round']}회차 매매 시작")
...
if order['round'] == 1:
    ...
    if not isinstance(account, dict):
        TA.send_tele(f"Pension2: 총 원화평가금 조회 불가로 종료합니다. ({account})")
        sys.exit(1)
    total_krw_asset = account['total_krw_asset']  # nass_amt (주식평가금 + D+2 현금 합계)
    message.append(f"Pension2총자산: {int(total_krw_asset):,}원 (주식:{int(account['stock_eval_amt']):,} + 현금:{int(account['cash_balance']):,})")

    # 당일 target 투자금액 및 목표 산출 및 저장하기
    this_krw_asset = total_krw_asset
    message.append(f"Pension2 이번 투자기준금: {int(this_krw_asset):,}원")
...
# 2회~12회차 투자 목표 매매 수량 불러오기
    ...
    message = []
    try:
        with open(PEN_target_path, 'r', encoding='utf-8') as f:
            target = json.load(f)
        target_code = list(target.keys())
        ...
        TA.send_tele(f"PEN_target.json 파일 오류: {e}")
        sys.exit(1)
...
if len(sell_code) == 0:
    message.append("Pension2:매도 종목 없음")

elif sell_split[0] > 0:
    message.append(f"Pension2: {order['round']}회차 - 매도 주문")
    for code, qty in sell.items():
        ...
        if price == 0 or not isinstance(price, int):
            TA.send_tele(f"Pension2: 현재가 조회 불가로 종료합니다. ({price})")
            sys.exit(1)
        ...
        for i in range(local_split_count):
            ...
            if order_info is None:
                message.append(f"Pension2 매도 오류: {code} API 응답 없음")
    ...
    message.append(f"Pension2: {order['round']}회차 - 매도 주문 없음 (분할횟수 0)")
...
if not isinstance(KRW, (int, float)):
    TA.send_tele(f"Pension2: 주문가능현금 조회 불가로 종료합니다. ({KRW})")
    sys.exit(1)
...
message.append(
    f"Pension2매수가능: {int(orderable_KRW):,}원 | 목표매수금: {int(target_KRW):,}원"
...
if target_KRW > orderable_KRW:
    ...
    message.append(f"Pension2 매수수량 조정 완료 (adjust_rate={adjust_rate:.4f})")
else:
    message.append("Pension2 매수가능금 충분 → 수량 조정 없음")
...
if len(buy_code) == 0:
    message.append("Pension2:매수 종목 없음")

elif len(buy_code) > 0 and buy_split[0] > 0:
    message.append(f"Pension2: {order['round']}회차 - 매수 주문")
    for code, qty in buy.items():
        ...
        if split_qty < 1:
            if qty < 1:
                message.append(f"Pension2 매수 스킵: {code} 수량 0주 (조정후 제거대상)")
                continue
            local_split_count = 1
        ...
        if not isinstance(price, int) or price == 0:
            TA.send_tele(f"Pension2: 현재가 없음으로 종료합니다. ({code})")
            sys.exit(1)        
        ...
        for i in range(local_split_count):
            ...
            if order_info is None:
                ...
                order_info = KIS.order_buy_KR(code, this_qty, order_price, "00")
            if order_info is None:
                message.append(f"Pension2 매수 오류: {code} {this_qty}주 {order_price:,}원 API 응답 없음")
            elif order_info.get("success"):
...
if order['round'] == 12:
    ...
    cancel_message = cancel_orders(side='all')
    message.append(cancel_message)
    message.append(f"Pension2 {order['date']} 리밸런싱 종료")
    ...
    for stock in stocks:
        ...
        if code not in target_code:
            message.append(f"Pension2: {stock['종목명']}는 target 리스트에 없는 이상 종목임.({stock['종목코드']})")
            continue
    ...
    # 전략결과 저장
    try:
        json_message = save_json(result, PEN_result_path, order)
        message.extend(json_message)
    except Exception as e:
        error_msg = f"PEN_result.json 저장 실패: {e}"
    ...
    # telegram message
    message.append(
        f"📊 Pension2 최종잔고 {tele_data['total']['date']}\n"
