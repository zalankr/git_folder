...
def get_balance():
    ...
    if USD_account:
        # USD: 외화예수금 → Total_balance 산출 (주식평가금과 이중산정 방지)
        USD = float(USD_account.get('deposit', 0))
        if USD <= 0:
            USD = float(USD_account.get('withdrawable', 0))
    else:
        USD = 0
    # ORD_PSBL_USD: TTTS3007R 해외주식 매수가능금액 조회
    # = 외화예수금 + T+2미결제매도금 - 당일이미사용금 → MTS '주문가능금액'과 동일
    # UPRO 현재가로 조회 (금액은 종목 무관, 수수료 계산 위해 가격 필요)
    _upro_price = KIS.get_US_current_price('UPRO')
    if isinstance(_upro_price, float) and _upro_price > 0:
        _avail = KIS.get_US_order_available('UPRO', _upro_price)
        ORD_PSBL_USD = _avail if _avail is not None else USD
    else:
        ORD_PSBL_USD = USD  # 가격 조회 실패 시 예수금으로 폴백
    ...
    return USD, USLA_balance, USLA_qty, USLA_price, HAA_balance, HAA_qty, HAA_price, Total_balance, ORD_PSBL_USD
...
if order_time['round'] == 1:
    ...
    # 계좌잔고 조회
    USD, USLA_balance, USLA_qty, USLA_price, HAA_balance, HAA_qty, HAA_price, Total_balance, ORD_PSBL_USD = get_balance()
    USD = float(USD)
    ...
    ## 만약 1월이라면 비중 리밸런싱
    order_time['month'] = 1 ############################################################3 최초 4월 실행 후 이 라인은 지울 것 #########
    ...
    # AVAILABLE_USD = TTTS3007R 주문가능금액 + 당일 매도예정금(방금 매도한 경우, 아직 API 미반영)
    SELL_AMOUNT = sum(
    ...
    AVAILABLE_USD = ORD_PSBL_USD + SELL_AMOUNT * (1 - fee_rate)
    message.append(
        f'USAA: 매수가능금액 산정\n'
        f'  현재예수금: ${USD:,.2f}\n'
        f'  주문가능금액(TTTS3007R): ${ORD_PSBL_USD:,.2f}\n'
        f'  매도예정금(당일분): ${SELL_AMOUNT:,.2f} (수수료 후: ${SELL_AMOUNT*(1-fee_rate):,.2f})\n'
        f'  합계: ${AVAILABLE_USD:,.2f}'
    )
