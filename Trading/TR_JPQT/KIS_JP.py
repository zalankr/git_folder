                ...
                for attempt in range(max_retry):
                    try:
                        ...
                        if attempt == max_retry - 1:
                            TA.send_tele(f"JP 잔고조회 연결 오류(최종): {conn_err}")
                            return None
                ...
                if result.get('rt_cd') != '0':
                    ...
                    if '변경' in msg1 or '다시 조회' in msg1:
                        ...
                        if snapshot_retry > MAX_SNAPSHOT_RETRY:
                            TA.send_tele(f"JP 미체결 스냅샷 재시도 {MAX_SNAPSHOT_RETRY}회 초과 → 중단 (누적 {len(unfilled_orders)}건)")
                            return unfilled_orders
                        wait_sec = 2 + snapshot_retry * 2
                        TA.send_tele(f"JP 미체결 스냅샷 무효 → {wait_sec}초 대기 후 재시도 {snapshot_retry}/{MAX_SNAPSHOT_RETRY}")
                        time.sleep(wait_sec)
                        ...
                        page_count = 0
                        continue
                    TA.send_tele(f"JP 미체결 조회 실패: {msg1}")   # ✅ 에러 메시지 출력 추가
                    return unfilled_orders
