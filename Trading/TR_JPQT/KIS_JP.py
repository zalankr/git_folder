...
class KIS_API:
        ...
        try:
            ...
            for stock in output1:
                ...
                stock_info = {
                    ...
                    'eval_amt': float(stock.get('frcr_evlu_amt2', 0)),     # 엔화 평가금액
    ...
    def get_JP_yen_balance(self) -> Optional[Dict]:
        """일본 엔화 예수금 (참조용 - 실제 주문가능금액은 get_JP_order_available 사용)"""
        path = "uapi/overseas-stock/v1/trading/inquire-present-balance"
        url = f"{self.url_base}/{path}"
        ...
        try:
            ...
            jpy_info = output2[0]
            deposit = float(jpy_info.get('frcr_dncl_amt_2', 0))
            return {
                'currency': jpy_info.get('crcy_cd', 'JPY'),
                'deposit': deposit,
                'withdrawable': float(jpy_info.get('frcr_drwg_psbl_amt_1', 0)),
                'sll_amt_smtl': float(jpy_info.get('frcr_sll_amt_smtl', 0)),
                'exchange_rate': float(jpy_info.get('frst_bltn_exrt', 0)),
                'krw_value': float(jpy_info.get('frcr_evlu_amt2', 0))
            }
        except:
            return None
