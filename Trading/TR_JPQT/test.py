...
# KIS instance 생성
# ============================================
key_file_path = "/var/autobot/TR_JPQT/kis63751991nkr.txt"       # 테스트계좌
token_file_path = "/var/autobot/TR_JPQT/kis63751991_token.json" # 테스트계좌
cano = "63751991"      # ← 계좌번호 수동 입력
acnt_prdt_cd = "01"
...
if order['round'] == 1 or order['round'] == 8:
    ...
    try:
        ...
        print(f"JPQT_stock.csv 파일 오류: {e}")
        sys.exit(1)
    ...
    if not isinstance(stocks_list, list):
        print(f"JPQT: 잔고 조회 불가로 종료합니다. ({stocks_list})")
        sys.exit(1)
    ...
    if orderable_jpy is None:
        print("JPQT: JPY 주문가능금액 조회 불가로 종료합니다.")
        sys.exit(1)
    ...
    if abs(total_weight - 1.0) > 0.01:
        print(f"JPQT 경고: CSV weight 합계 = {total_weight:.3f} (1.0 아님). 계속 진행합니다.")
    ...
    for ticker in target_code:
        ...
        if not isinstance(price, float) or price <= 0:
            print(f"JPQT: {ticker} 현재가 조회 불가로 종료합니다. ({price})")
            sys.exit(1)
    ...
    try:
        ...
        print(f"JPQT_target.json 파일 오류: {e}")
        sys.exit(1)
    target_code = list(target.keys())
...
for k, v in target.items():
    if k == "CASH":
        continue
    print(f"{k}: {v['name']} ({v['weight']:.2f}) {v['categories']} {v['target_invest']:.0f} {v['target_qty']} {v['current_price']:.0f}")
...
if len(sell_code) == 0:
    ...
    print(f"JPQT: {order['round']}회차 - 매도 주문")
    for ticker, qty in sell.items():
        ...
        if split_qty < 100:
            print(f"JPQT 매도 스킵: {ticker} 수량 {qty}주 (100주 미만)")
            continue
        ...
        for i in range(local_split_count):
            ...
            print(f"JPQT 매도 주문: {ticker} {quantity}주 ¥{order_price:,}")
    ...
    print(f"JPQT: {order['round']}회차 매도 스킵 - 미처분 잔량: {list(sell.keys())}")
...
for ticker, qty in buy.items():
    price = KIS.get_JP_current_price(ticker)
    if not isinstance(price, float) or price <= 0:
        print(f"JPQT 매수가능: ¥{orderable_JPY:,.0f} | 목표매수금: ¥{target_JPY:,.0f}")(f"JPQT: {ticker} 현재가 조회 불가로 종료합니다. ({price})")
        sys.exit(1)
...
if target_JPY > orderable_JPY:
    ...
    print(f"JPQT 매수가능: ¥{orderable_JPY:,.0f} | 목표매수금: ¥{target_JPY:,.0f}")
    print(f"JPQT 매수수량 조정 완료 (adjust_rate={adjust_rate:.4f})")
else:
    print(f"JPQT 매수가능: ¥{orderable_JPY:,.0f} | 목표매수금: ¥{target_JPY:,.0f}")
    print("JPQT 매수가능금 충분 → 수량 조정 없음")
...
if len(buy_code) == 0:
    print(f"JPQT 매수가능: ¥{orderable_JPY:,.0f} | 목표매수금: ¥{target_JPY:,.0f}")("JPQT: 매수 종목 없음")
    ...
    for ticker, qty in buy.items():
        ...
        if split_qty < 100:
            if qty < 100:
                print(f"JPQT 매수가능: ¥{orderable_JPY:,.0f} | 목표매수금: ¥{target_JPY:,.0f}")(f"JPQT 매수 스킵: {ticker} 수량 {qty}주 (100주 미만)")
                continue
            local_split_count = 1
        ...
        if not isinstance(price, float) or price <= 0:
            print(f"JPQT 매수가능: ¥{orderable_JPY:,.0f} | 목표매수금: ¥{target_JPY:,.0f}")(f"JPQT: {ticker} 현재가 없음으로 종료합니다.")
            sys.exit(1)
        ...
        for i in range(local_split_count):
            ...
            # ★ 일본주식: 정수 가격
            order_price = int(round(price * local_split_price[i], 0))
            print(f"JPQT 매수 주문: {ticker} {quantity}주 ¥{order_price:,}")
...
if order['round'] == 14:
    ...
    stocks = KIS.get_JP_stock_balance()
    if not isinstance(stocks, list):
        print(f"JPQT: 잔고 조회 불가로 종료합니다. ({stocks})")
        sys.exit(1)
    ...
    if not isinstance(final_stocks, list):
        print(f"JPQT: 최종 잔고 조회 불가로 종료합니다. ({final_stocks})")
        sys.exit(1)
    ...
    if final_jpy is None:
        print("JPQT: 최종 JPY 주문가능금액 조회 불가로 종료합니다.")
        sys.exit(1)
