...
def make_position(upbit):
    ...
    Last_day_balance = TR_data.get("Last_day_balance", 0.0)
    Last_month_balance = TR_data.get("Last_month_balance", 0.0)
    Last_year_balance = TR_data.get("Last_year_balance", 0.0)
    Daily_return = TR_data.get("Daily_return", 0.0)
    Monthly_return = TR_data.get("Monthly_return", 0.0)
    Yearly_return = TR_data.get("Yearly_return", 0.0)
...
# 직전 2시간 체결 주문 확인 함수
def check_filled_orders_last_hour(upbit, ticker: str, Upmessage: list):
    """
    Args:
        upbit: Upbit 객체
    ...
    try:
        ...
        for filled_order in filled_orders:
            ...
            # 12시간 이내의 주문만 확인 (타임존 자동 변환되어 비교)
            if created_at < two_hour_ago:
                continue
        ...
        Upmessage.append(f"Upbit: {ticker} 직전 2시간 체결 요약: 사용 KRW {total_krw_used:.0f}원")
        Upmessage.append(f"체결량 {total_volume_filled:.8f}개")
        
        return total_krw_used, total_volume_filled, Upmessage
    
    except Exception as e:
        Upmessage.append(f"Upbit {ticker} 체결 확인 오류: {e}")
        return 0.0, 0.0, Upmessage

# ETH와 BTC 모두의 직전 2시간 매수 체결 내역을 확인하는 래퍼 함수
