...
# 라이브러리 설치: pip install gspread google-auth
...
try:
    if TR_time[1] == 8: # 8:58 8분할 매매시에만 실행
        ...
        message.append(f"Upbit: {TR_time[0]} Upbit trade 당일 전략 data 생성완료")
    else:
        pass

except Exception as e:
    message.append(f"Upbit: {TR_time[0]} \nUpbit trade 당일 전략생성 시 예외 오류: {e}")
...
try:
    if TR_time[1] in range(1, 9): # 8분할매매 주문 실행(0은 제외)
        ...
        if result:  # 리스트가 비어있지 않으면 True
            ...
            message.append(f"Upbit: {TR_time[0]}, \n{TR_time[1]}회차 분할매매, \n취소할 주문이 없습니다.")
        time_module.sleep(0.5) # 타임 슬립 0.5초

        # 당일의 TR_data.json 파일 불러오기     
        try:
