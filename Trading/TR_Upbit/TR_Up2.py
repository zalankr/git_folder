...
try:
    if TR_time[1] == 8: # 8:58 8분할 매매시에만 실행
        ...
        # Upbit 당일 전략 데이터 만들기
        TR_data_today = UP.make_position(upbit)
        print(TR_data_today)
    
        # 데이터 업데이트 (기존 데이터 + 새 데이터 병합)
        TR_data.update(TR_data_today)
...
try:
    if TR_time[1] in [8, 7, 6, 5, 4, 3, 2, 1]: # 8,7,6,5,4,3,2,1 분할매매 주문 실행(0은 제외)
        ...
        Last_ETH_weight = TR_data["Last_ETH_weight"]
        Last_BTC_weight = TR_data["Last_BTC_weight"]
...
try:
    if TR_time[1] == 1:
        ...
        Last_day_Total_balance = TR_data["Last_day_Total_balance"]
        Last_month_Total_balance = TR_data["Last_month_Total_balance"]
        Last_year_Total_balance = TR_data["Last_year_Total_balance"]

        # 일, 월, 연 수익률 #
        Daily_return = (Total - Last_day_Total_balance) / Last_day_Total_balance * 100
        Daily_return = float("{:.2f}".format(Daily_return))
        Monthly_return = (Total - Last_month_Total_balance) / Last_month_Total_balance * 100
        Monthly_return = float("{:.2f}".format(Monthly_return))
        Yearly_return = (Total - Last_year_Total_balance) / Last_year_Total_balance * 100
