...
class BinanceT:
    ...
    def make_position(self): # 어제 저장된 binance_data.json 파일을 불러서 오늘 포지션으로 변경하는 함수
        ...
        if BTC_weight == 1.0 :
            ...
            if MA45signal == "Buy" and MA120signal == "Buy":
                position = {"position": "Buy full", "BTC_weight": 1.0, "BTC_target": ((USDT*0.9995)/price), "CASH_weight": 0.0, "Invest_quantity": USDT * 1.0}
            elif MA45signal == "Sell" and MA120signal == "Sell":
                position = {"position": "Hold state", "BTC_weight": 0.0, "BTC_target": 0.0, "CASH_weight": 1.0, "Invest_quantity": 0.0}
            else:
                position = {"position": "Buy half", "BTC_weight": 0.5, "BTC_target": ((USDT*0.5*0.9995)/price) * 0.5, "CASH_weight": 0.5, "Invest_quantity": USDT * 0.5}
