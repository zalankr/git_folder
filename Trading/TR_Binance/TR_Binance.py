...
access, secret = read_keys(KEY_FILE)
...
btc_spot = balance['total'].get('BTC', 0)
usdt_spot = balance['total'].get('USDT', 0)

# 2) Funding Wallet (Flexible Earn 포함) — 올바른 implicit API 호출 (POST)
funding_usdt = None
payload = {"asset": "USDT"}  # 필요하면 제거해도 전체 반환

# 가능한 메서드 이름들을 순서대로 시도
candidates = [
    "sapi_post_asset_get_funding_asset",
    "sapiPostAssetGetFundingAsset",
    "sapi_post_asset_get_fundingasset",  # 드문 변형
]
for name in candidates:
    fn = getattr(binance, name, None)
    if callable(fn):
        try:
            resp = fn(payload)  # ccxt가 서명/타임스탬프를 처리합니다
            # 응답 형태에 따라 안전하게 파싱
            if isinstance(resp, dict):
                if resp.get("asset") == "USDT":
                    funding_usdt = float(resp.get("free", 0))
                else:
                    # 어떤 구현은 {'data':[...]} 같은 형태일 수 있음
                    for item in resp.get("data", []):
                        if item.get("asset") == "USDT":
                            funding_usdt = float(item.get("free", 0))
                            break
...
# 출력
print("Spot BTC:", btc_spot)
print("Spot USDT:", usdt_spot)
print("Funding USDT (free, Flexible Earn 등):", funding_usdt)


# 시간확인 조건문
now, current_time, TR_time = myBinance.what_time()

# # If 8:58 Trading 5분할(첫 번째)에만 전일 Upbit_data json읽고 Signal계산, 투자 금액 산출 후 다시 저장
# try:
#     if TR_time[1] == 5: # 8:59 5분할 매매시에만 실행
#         # 기존 주문 모두 취소
