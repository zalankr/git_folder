...
# KIS instance 생성
key_file_path = "/var/autobot/TR_ISAYS/kis43680827lys.txt"
token_file_path = "/var/autobot/TR_ISAYS/kis43680827_token.json"
cano = "43680827"
acnt_prdt_cd = "01"
...
def health_check():
    ...
    if checkday == False:
        checks.append("ISAYS체크: 거래일이 아닙니다.")
...
def save_json(data, path, order):
    ...
    try:
        ...
        backup_path = f"/var/autobot/TR_ISAYS/backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        try:
...
def cancel_orders(side: str="all"):
    ...
    if isinstance(summary, dict):
        cancel_message = f"ISAYS: {summary['success']}/{summary['total']} 주문 취소 성공"
    else:
...
if order['round'] == 1:
    ...
    message.append(f"ISAYS 이번 투자기준금: {int(this_krw_asset):,}원 "
...
# 2회~12회차 투자 목표 매매 수량 불러오기
    ...
    message = []
    try:
        with open(ISAYS_target_path, 'r', encoding='utf-8') as f:
            target = json.load(f)
        target_code = list(target.keys())
        ...
        TA.send_tele(f"ISAYS_target.json 파일 오류: {e}")
        sys.exit(1)
...
if len(sell_code) == 0:
    message.append("ISAYS:매도 종목 없음")

elif sell_split[0] > 0:
    message.append(f"ISAYS: {order['round']}회차 - 매도 주문")
    for code, qty in sell.items():
        ...
        for i in range(local_split_count):
            ...
            if order_info is None:
                message.append(f"ISAYS 매도 오류: {code} API 응답 없음")
...
message.append(
    f"ISAYS 매수가능: {int(orderable_KRW):,}원 | 목표매수금: {int(target_KRW):,}원"
...
if target_KRW > orderable_KRW:
    ...
    message.append(f"ISAYS 매수수량 조정 완료 (adjust_rate={adjust_rate:.4f})")
else:
    message.append("ISAYS 매수가능금 충분 → 수량 조정 없음")
...
if len(buy_code) == 0:
    message.append("ISAYS:매수 종목 없음")

elif len(buy_code) > 0 and buy_split[0] > 0:
    message.append(f"ISAYS: {order['round']}회차 - 매수 주문")
    for code, qty in buy.items():
        ...
        if split_qty < 1:
            if qty < 1:                   # qty 자체가 0이면 주문 스킵
                message.append(f"ISAYS 매수 스킵: {code} 수량 0주 (조정후 제거대상)")
                continue
            local_split_count = 1
        ...
        for i in range(local_split_count):
            ...
            if order_info is None:
                message.append(f"ISAYS 매수 오류: {code} {split_qty}주 {order_price:,}원 API 응답 없음")
            elif order_info.get("success"):
...
if order['round'] == 12:
    ...
    cancel_message = cancel_orders(side='all')
    message.append(cancel_message)
    message.append(f"ISAYS {order['date']} 리밸런싱 종료")
