...
#
#   crontab 권장 (UTC, 평일):
#   13 0-6 * * 1-5  → UTC 00:13 (KST 09:13, 7분할)
# 
SPLIT_SCHEDULE = [
    (0,  13),   # 1번째 분할  (UTC 00:13 = KST 09:13)
    (1,  13),   # 2번째 분할  (UTC 01:13 = KST 10:13)
    (2,  13),   # 3번째 분할  (UTC 02:13 = KST 11:13)
    (3,  13),   # 4번째 분할  (UTC 03:13 = KST 12:13)
    (4,  13),   # 5번째 분할  (UTC 04:13 = KST 13:13)
    (5,  13),   # 6번째 분할  (UTC 05:13 = KST 14:13)
    (6,  13),   # 7번째 분할  (UTC 06:13 = KST 15:13)
]
...
#     UTC = KST - 9시간
#
#   # KRX 금현물 분할 매매 - 평일(월~금) 1일 7회 분할 (UTC 시각)
#     13 0-6 * * 1-5  /usr/bin/python3 /var/autobot/TR_GOLD/GOLD_TR.py
#   # KST 09:13~15:13 평일 매수/매도 실행
#
#   ※ SPLIT_SCHEDULE 의 (시,분) 7개와 위 cron 시각 7개를 일치시킬 것 (둘 다 UTC)
#   ※ crontab의 요일(1-5)도 UTC 기준. 한국 월~금 장중 시간대(UTC 00~07시)는
#     UTC 요일과 KST 요일이 동일하므로 1-5 그대로 사용 가능.
#   ※ 며칠간만 매매 시 날짜 필드로 제한 (날짜도 UTC 기준):
#     13 0-6 19-21 * 1-5  → 매월 19~21일 평일 KST 09:13부터 15:13까지 실행
# ===========================================================================
