...
TopCoinList = myUpbit.GetTopCoinList(interval="day", top=5)
    ...
    try: 
        ...
        if myUpbit.IsHasCoin(balances,ticker) == True:
            ...
            if rsi60_min >= 70.0 and revenu_rate >= 1.0:
                ...
                print(f"myUpbit.CancelCoinOrder(upbit,{ticker})")
                myUpbit.SendMessage(f"myUpbit.CancelCoinOrder(upbit,{ticker})")
                
                #최대코인매수금액의 1/4보다 작다면 전체 시장가 매도 
                if NowCoinTotalMoney < (CoinMaxMoney / 4.0):
                    #시장가 매도를 한다.
                    print(f"balances = myUpbit.SellCoinMarket(upbit,{ticker},upbit.get_balance(ticker))")
                    myUpbit.SendMessage(f"balances = myUpbit.SellCoinMarket(upbit,{ticker},upbit.get_balance(ticker))")

                #최대코인매수금액의 1/4보다 크다면 절반씩 시장가 매도 
                else:
                    #시장가 매도를 1/2 만큼한다.
                    print(f"balances = myUpbit.SellCoinMarket(upbit,{ticker},upbit.get_balance(ticker) / 2.0)")
                    myUpbit.SendMessage(f"balances = myUpbit.SellCoinMarket(upbit,{ticker},upbit.get_balance(ticker) / 2.0)")
            ...
            if rsi60_min_before <= 30.0 and rsi60_min > 30.0:
                ...
                if Total_Rate <= 50.0:
                    ...
                    myUpbit.SendMessage(f"myUpbit.CancelCoinOrder(upbit,{ticker})")
                     ...
                     #시장가 매수를 한다.
                    print(f"balances = myUpbit.BuyCoinMarket(upbit,{ticker},WaterEnterMoeny)")
                    myUpbit.SendMessage(f"balances = myUpbit.BuyCoinMarket(upbit,{ticker},WaterEnterMoeny)")
