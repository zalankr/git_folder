...
HKQT_rebal_path = "/var/autobot/TR_HKQT/HKQT_rebal.json"
...
if order['round'] == 14:
    ...
    # HKQT_daily.json 저장
    try:
        json_message = save_json(rebal_data, HKQT_rebal_path, order)
        message.extend(json_message)
    except Exception as e:
    ...
    rebal = {
        ...
        "total_stocks":    f"HK${rebal_data['total_stocks']:,.2f}",
        "total_cash":      f"HK${rebal_data['total_cash']:,.2f}",
        "total_asset":     f"HK${rebal_data['total_asset']:,.2f}",
        "total_asset_ret": f"{float(rebal_data['total_asset_ret']*100):.2f}%"
    }

    for category, stocks_list in result.items():
        category_balance = sum(float(item['balance']) for item in stocks_list)
        rebal[category]          = f"HK${category_balance:,.2f}"
