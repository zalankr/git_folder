...
try:
    ...
    print("HKQT: 이미 실행 중입니다.")
    sys.exit(0)

# ============================================
# KIS instance 생성
# ============================================
key_file_path = "/var/autobot/TR_HKQT/kis63604155nkr.txt"       # 트레이딩 계좌(수수료할인)
token_file_path = "/var/autobot/TR_HKQT/kis63604155_token.json" # 트레이딩 계좌(수수료할인)
...
def order_time(day=1):
    ...
    crontab (UTC):
      오전장 3회: 37 1,2,3 리밸런싱일,리밸런싱일+1 반기월 *
        → KST 10:37, 11:37, 12:37
      오후장 4회: 7 5,6,7 리밸런싱일,리밸런싱일+1 반기월 *
                  37 5 리밸런싱일,리밸런싱일+1 반기월 *
        → KST 14:07, 14:37, 15:07, 16:07
      (총 7회차 per day × 2일 = 14회차)
    
    실제 crontab 예시 (UTC):
      37 1,2,3 리밸런싱일,리밸런싱일+1 반기월 *   → 오전 3슬롯
      7 5,6,7 리밸런싱일,리밸런싱일+1 반기월 *     → 오후 3슬롯
      37 5 리밸런싱일,리밸런싱일+1 반기월 *         → 오후 1슬롯
      (총 7회차 per day)
    """
...
if order['round'] == 1 or order['round'] == 8:
    ...
    try:
        ...
        print(f"HKQT_stock.csv 파일 오류: {e}")
        sys.exit(1)
    ...
    if not isinstance(stocks_list, list):
        print(f"HKQT: 잔고 조회 불가로 종료합니다. ({stocks_list})")
        sys.exit(1)
    ...
    orderable_hkd = 100000.0
    if orderable_hkd is None:
        print("HKQT: HKD 주문가능금액 조회 불가로 종료합니다.")
        sys.exit(1)
    ...
    if abs(total_weight - 1.0) > 0.01:
        print(f"HKQT 경고: CSV weight 합계 = {total_weight:.3f} (1.0 아님). 계속 진행합니다.")
    ...
    for ticker in target_code:
        ...
        if not isinstance(price, float) or price <= 0:
            print(f"HKQT: {ticker} 현재가 조회 불가로 종료합니다. ({price})")
            sys.exit(1)
    ...
    try:
        ...
        print(f"HKQT_target.json 파일 오류: {e}")
        sys.exit(1)
    target_code = list(target.keys())
...
for stock in stocks:
    ...
    hold[ticker] = {
        "name": stock["name"],
        "hold_balance": stock["eval_amt"],      # HKD 평가금액
...
print("sell", sell)
print("\n".join([f"{k} : {v}" for k, v in sell.items()]))
print("buy", buy)
print("\n".join([f"{k} : {v}" for k, v in buy.items()]))
...
if len(sell_code) == 0:
    ...
    for ticker, qty in sell.items():
        ...
        for i in range(local_split_count):
            ...
            # order_info, order_msgs = KIS.order_sell_HK(ticker, quantity, order_price)
            # if order_info is None:
            #     time_module.sleep(2)
            #     order_info, order_msgs = KIS.order_sell_HK(ticker, quantity, order_price)
            # if order_info is None:
            #     message.append(f"HKQT 매도 오류: {ticker} {quantity}주 HK${order_price:,.2f} API 응답 없음")
            # elif order_info.get("success"):
            #     message.append(
            #         f"매도 {ticker} {quantity}주 HK${order_price:,.2f} "
            ...
            #     )
            print(f"매도 {ticker} {quantity}주 HK${order_price:,.2f}")
...
if target_HKD > orderable_HKD:
    ...
    print(f"HKQT 매수수량 조정 완료 (adjust_rate={adjust_rate:.4f})")
else:
    print("HKQT 매수가능금 충분 → 수량 조정 없음")
...
if len(buy_code) == 0:
    ...
    print(f"HKQT: {order['round']}회차 - 매수 주문")
    for ticker, qty in buy.items():
        ...
        if split_qty < 1:
            if qty < 1:
                print(f"HKQT 매수 스킵: {ticker} 수량 0주 (조정후 제거대상)")
                continue
            local_split_count = 1
        ...
        if not isinstance(price, float) or price <= 0:
            print(f"HKQT: {ticker} 현재가 없음으로 종료합니다.")
            sys.exit(1)
        ...
        for i in range(local_split_count):
            ...
            # order_info, order_msgs = KIS.order_buy_HK(ticker, quantity, order_price)
            # if order_info is None:
            #     time_module.sleep(2)
            #     order_info, order_msgs = KIS.order_buy_HK(ticker, quantity, order_price)
            # if order_info is None:
            #     message.append(f"HKQT 매수 오류: {ticker} {quantity}주 HK${order_price:,.2f} API 응답 없음")
            # elif order_info.get("success"):
            #     message.append(
            #         f"매수 {ticker} {quantity}주 HK${order_price:,.2f} "
            #         f"주문번호:{order_info.get('order_number','')}"
            #     )
            # else:
            #     message.append(
            #         f"매수 실패 {ticker} {quantity}주 HK${order_price:,.2f}: "
            #         f"{order_info.get('error_message','')}"
            #     )
            print(f"매수 {ticker} {quantity}주 HK${order_price:,.2f}")
print("\n".join(message))
message = []
...
if order['round'] == 14:
    ...
    print(f"HKQT {order['date']} 리밸런싱 종료")
    ...
    stocks = KIS.get_HK_stock_balance()
    if not isinstance(stocks, list):
        print(f"HKQT: 잔고 조회 불가로 종료합니다. ({stocks})")
        sys.exit(1)
    ...
    for category, stocks_list in result.items():
        print(f"{order['date']}일 리밸런싱 전략명:{category} 결과")
    ...
    if not isinstance(final_stocks, list):
        print(f"HKQT: 최종 잔고 조회 불가로 종료합니다. ({final_stocks})")
        sys.exit(1)
    ...
    if final_hkd is None:
        print("HKQT: 최종 HKD 주문가능금액 조회 불가로 종료합니다.")
        sys.exit(1)
