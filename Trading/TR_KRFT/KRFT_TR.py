...
HEDGE3_T1_PBR     = 3.1         # 1단계 진입 PBR (30%)
HEDGE3_T2_PBR     = 3.65        # 2단계 진입 PBR (60%)
...
def compute_hedge3_daily_signal(pbr: float, positions: dict) -> dict:
    """
    Hedge3 daily 신호:
      - 진입: PBR ≥ HEDGE3_T1_PBR 즉시 T1 비중,
              PBR ≥ HEDGE3_T2_PBR 즉시 T2 비중
      - 청산: 진입 후 갱신한 peak_pbr 의 HEDGE3_EXIT_RATIO 이하로
              떨어지면 즉시 (peak는 일별 갱신)
    ...
    # 새 peak = max(기존 peak, 오늘 pbr) — T1 미달 시는 갱신 안 함
    new_peak = max(cur_peak, pbr) if active or pbr >= HEDGE3_T1_PBR else cur_peak

    # 1) 청산 조건 (active 일 때만)
    if active and new_peak > 0 and pbr <= new_peak * HEDGE3_EXIT_RATIO:
        return {
            "action":   "close",
            "ratio":    0.0,
            "active":   False,
            "peak_pbr": 0.0,
            "reason":   f"Hedge3 청산: PBR {pbr:.4f} ≤ peak {new_peak:.4f}×"
                        f"{HEDGE3_EXIT_RATIO:.4f} = {new_peak*HEDGE3_EXIT_RATIO:.4f}",
        }

    # 2) 목표 비중 결정 (단조 비감소)
    if pbr >= HEDGE3_T2_PBR:
        target_ratio = HEDGE3_T2_RATIO
    elif pbr >= HEDGE3_T1_PBR:
        target_ratio = HEDGE3_T1_RATIO
    else:
        target_ratio = 0.0
    ...
    return {
        ...
        "active":   False,
        "peak_pbr": cur_peak,
        "reason":   f"Hedge3 미발동 (PBR {pbr:.4f} < {HEDGE3_T1_PBR})",
    }
...
def run_hedge3_daily() -> dict:
    ...
    if result["positions"]["hedge3"].get("active") or pbr >= HEDGE3_T1_PBR:
