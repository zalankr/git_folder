...
ticker = ['SHY', 'IEF', 'TLT', 'LQD', 'HYG', 'PDBC', 'TIP', 'BIL', 'QQQ']
ticker0 = ['SHY', 'IEF', 'TLT', 'LQD', 'HYG', 'PDBC', 'TIP', 'BIL', 'QQQ', 'CASH']
...
m = pd.DataFrame(Momentum, columns=['Ticker', 'Momentum'])
c = pd.DataFrame({'Ticker' : ["CASH"],'Momentum' : ['']})
mc = pd.concat([m, c], axis=0, ignore_index=True)

TR.insert(loc=4, column='Momentum', value=mc.Momentum)
time.sleep(0.01)
...
if int(month) % 2 != 0 :
    # Momentum Score 산출
    Mom_S = m  
    Mom_S.loc[:, 'rank'] = Mom_S.loc[:, 'Momentum'].rank(ascending=False, 
                                                          method='max', 
                                                          pct=False, 
                                                          na_option='bottom')        
    Mom_S = Mom_S.sort_values(by=['rank'], ascending=True, ignore_index=True, inplace=False)
    ...
    for i in range(3) :
        ...
        if o >= 0 and k != 'BIL' :
            Target.append(k)
            Weight.append(1/3*0.99)
                       
    if len(Target) < 3 :
        a = (3-len(Target)) * 1/3
        Weight.append(a*0.99+0.01)
        Target.append('CASH')        
    ...
    TR['매수량'] = buy
    TR['매수가'] = price
    TR.insert(loc=16, column='매수금액', value=(TR.매수량*TR.매수가)+(TR.매수량*TR.매수가*tax))
    ...
    d9 = round((float(d2) / float(d1) - 1), 4)
    ...
    # 라인알러트 보내기
    # line_alert.SendMessage(str(year) + '-' + str(month) +' BCAA')
    ...
    # except : 
    #     line_alert.SendMessage('티커 갯수 이하')
...
### 짝수 전월 유지 ### 1월 작성 후        
    ...
    # 투입금액 구하기
    TR.insert(loc=9, column='투입금액', value=TR.현재평가금)
    
    # 목표량 구하기
    TR.insert(loc=10, column='목표량', value=TR.전월수량)
    ...
    # 최종항목 구하기
    TR.insert(loc=17, column='최종수량', value=TR.전월수량)
    TR['최종주가'] = price
    TR.insert(loc=19, column='최종평가금', value=(TR.현재평가금))
    ...
    # 라인알러트 보내기
    # line_alert.SendMessage(str(year) + '-' + str(month) +' BCAA')
    # line_alert.SendMessage('전월 동일 비중')
...
# worksheet.set_dataframe(TR, "A1", include_column_header=True)
...
worksheet.update(values=[TR.columns.values.tolist()] + TR.values.tolist(), range_name="A1:T23")
worksheet.update_cell(24, 1, "* 수정 > 찾기 및 바꾸기 > 찾기: ' | 바꾸기 : 공란 > 체크+수식내검색 > 모두 바꾸기 & 말일신호 익일거래 & 실제 수량 및 금액 기입 시 ,표 없이 | 추가 수정주가 확인")
